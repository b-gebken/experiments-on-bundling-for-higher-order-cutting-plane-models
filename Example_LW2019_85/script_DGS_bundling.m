...
f = @(x) lw2019_85_f(x,g_arr,H_cell,c_arr);
grad_f = @(x) lw2019_85_grad(x,g_arr,H_cell,c_arr);
hess_f = @(x) lw2019_85_hess(x,g_arr,H_cell,c_arr);
...
for j = 1:j_max
    ...
    signs_arr = zeros(k,num_sample_pts(j));

    W_grad_min = zeros(n,num_sample_pts(j));
    for i = 1:num_sample_pts(j)
        [~,signs_arr(:,i)] = f(bundle_cell{j}.sample_pts(:,i));
        W_grad_min(:,i) = lw2019_85_grad_signs(x_min,g_arr,H_cell,c_arr,signs_arr(:,i));
    end
